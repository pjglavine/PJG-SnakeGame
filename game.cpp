...
Game::Game(std::size_t grid_width, std::size_t grid_height)
    : snake(grid_width, grid_height),
      engine(dev()),
      random_w(1, static_cast<int>(grid_width-2)), //modified to keep food on screen.
      random_h(1, static_cast<int>(grid_height-2)) { // and prevent food from spawning against wall
  PlaceFood();
  PlacePoison();
}
...
void Game::Run(Controller const &controller, Renderer &renderer,
  ...
  //music.SoundPlayback();
  Audio audio;
  audio.GameMusic(true);

  while (running) {
    frame_start = SDL_GetTicks();

    // Input, Update, Render - the main game loop.
    controller.HandleInput(running, snake, audio);
    if (snake.alive){
      Update(audio);
    } else {
...
// Check if there is food spawned in cell.
